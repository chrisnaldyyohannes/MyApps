...
import com.example.myapps.Message
import com.example.myapps.R
import com.example.myapps.User
import com.example.myapps.adapter.MessagesAdapter
import com.example.myapps.util.DATA_CHATS
import com.example.myapps.util.DATA_MESSAGES
...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (chatId.isNullOrEmpty() || userId.isNullOrEmpty() || imageUrl.isNullOrEmpty() || otherUserId.isNullOrEmpty()){
            Toast.makeText(this,"no chat",Toast.LENGTH_SHORT).show()
            finish()
        }
        chatDatabase = FirebaseDatabase.getInstance().reference.child(DATA_CHATS)
        messageAdapter = MessagesAdapter(ArrayList(),userId!!)
        messageRV.apply {
